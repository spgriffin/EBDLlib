...
; :Author: liao46
;-
PRO hdf2binary_dsi,workspace_in,workspace_out,prefix_in,prefix_out,start_year,end_year,dataset_field
  COMPILE_OPT IDL2  
  ...
  FOR iyear=start_year,end_year,1 DO BEGIN
    ...
    for day=0,382,1 do begin
      ...
      if(file_test(filename_in) eq 1) then begin
        ...
        ENDIF
        global_data=MAKE_ARRAY(7200,3600,value=-9999.0,/float)
        ;this is the special case for DSI data!!!!
        global_data[*,200:2999]=data
        filename_out=year_out + !backslash + prefix_in+year_str+day_str + binary_extension
        OPENW,lun,filename_out,/get_lun
        WRITEU,lun,global_data
        FREE_LUN, lun
        hdr_out=year_out + !backslash + prefix_in+year_str+day_str + hdr_extension
        OPENW,lun,hdr_out,/get_lun
        PRINTF,lun,'ncols ',7200
        PRINTF,lun,'nrows ',3600
        PRINTF,lun,'xllcorner ',-180.0
        PRINTF,lun,'yllcorner ', -90.0
        PRINTF,lun,'cellsize ',0.05
