PRO mosaic_resample, workspace_in, workspace_out_mosaic, workspace_out_resample, prefix_in,prefix_out,file_extension,pixel_size,band_pos,fill_value,data_type ,factors, start_year, end_year
  ...
  FOR iyear = start_year, end_year, 1 DO BEGIN
    ...
    FOR day = 0, 382, 1 DO BEGIN
      ...
      folder_in=year_in + !backslash
      filename_regex = prefix_in+year_str + day_str+'*'+file_extension
      match_files = FILE_SEARCH(folder_in, filename_regex, /FULLY_QUALIFY_PATH, COUNT = file_count)
      IF(file_count GT 1) THEN BEGIN
        ...
        filename_out2 =  year_out2+!backslash+ prefix_out+year_str+ day_str+tiff_extension
